...
has no access to $\kappa$, to the language model, or to the
deployed detector as a queryable oracle at detection time
(\autoref{sec:discussion} discusses the adaptive-oracle adversary
that lies outside this model). The verifier sees only token text and runs in $O(n)$ hash
operations.
