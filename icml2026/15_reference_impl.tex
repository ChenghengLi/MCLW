...
Algorithm~\ref{alg:detect} for clockwork MCL. It takes a list of integer
...
(cf.~Proposition~\ref{prop:kregular}) require replacing the successor check
...
def sha_state(key: bytes, token_id: int, S: int) -> int:
    """SHA-256-based state assignment sigma_k(t) = H(k || t) mod S."""
...
of SHA-256 (Theorem~\ref{thm:security}). In practice an auditor and a
